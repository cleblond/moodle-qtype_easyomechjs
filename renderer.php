...
class qtype_easyomechjs_renderer extends qtype_renderer {
	...
	$marvinjspath = $marvinjsconfig->path;
        ...
        if ($placeholder) {
            $toreplace = html_writer::tag('span',
                                      get_string('enablejavaandjavascript', 'qtype_easyomechjs'),
                                      array('class' => 'ablock'));
        ...
        if (!$placeholder) {
            $answerlabel = html_writer::tag('span', get_string('answer', 'qtype_easyomechjs', ''),
	...
	$marvinjspath = "http://" . $_SERVER['HTTP_HOST'] . $marvinjsconfig->path;
        ...
        $strippedanswerid = "stripped_answer".$qa->get_slot();
        $PAGE->requires->js_init_call('M.qtype_easyomechjs.insert_easyomechjs_applet',
                                      array($toreplaceid,
                                            $name,
                                            $appletid,
                                            $topnode,
